...
| Componente | Número de Parte | Descripción |
|------------|-----------------|-------------|
...
```plaintext
┌─────────────────────────────────────────────────────────────────────┐
│                      Sistema de Entorno Espacial                     │
└───────────────────────────────┬─────────────────────────────────────┘
                              │
  ┌───────────────────────────┼───────────────────────────┐
  │                           │                           │
┌───▼───────────┐         ┌─────▼─────────┐          ┌──────▼──────┐
│ Sistemas de   │         │ Unidad de     │          │Infraestructura│
│   Sensores    │         │Procesamiento  │          │              │
└───┬───────────┘         └─────┬─────────┘          └──────┬──────┘
  │                           │                           │
  ├───────────────┐           ├───────────────┐           ├───────────────┐
┌───▼───────────┐ ┌─▼───────────────┐ ┌─────────▼───────┐ ┌─▼─────────────┐ ┌─▼───────────────┐
│Sensores de    │ │Detección de     │ │Procesamiento    │ │Gestión de     │ │Red y Seguridad  │
│  Radiación    │ │  Escombros      │ │   Cuántico      │ │  Potencia     │ │                 │
└───┬───────────┘ └─────┬───────────┘ └─────────┬───────┘ └─┬─────────────┘ └─────┬───────────┘
  │                   │                       │            │                     │
  ├───────────────┐   ├───────────────┐       ├───────────┐ ├─────────────┐     ├───────────────┐
┌───▼───────────┐ ┌─▼───────────────┐ ┌─▼───────────────┐ ┌─▼─────────────┐ ┌─────▼───────────┐
│Detectores     │ │Sistema Óptico   │ │Sistema Criogénico│ │UPS y Distribución│ │Interfaz INFRANET│
│  Múltiples    │ │    LIDAR        │ │                 │ │                 │ │                 │
└───────────────┘ └─────────────────┘ └─────────────────┘ └───────────────┘ └───────────────────┘
```
...
```plaintext
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│Adquisición│───▶│Procesamiento│───▶│ Análisis  │───▶│Predicción │───▶│  Acción   │
│  de Datos │    │ de Señal  │    │ Cuántico │    │           │    │           │
└──────────┘    └──────────┘    └──────────┘    └──────────┘    └──────────┘
     ▲               │               │               │               │
     │               │               │               │               │
     └───────────────┴───────────────┴───────────────┴───────────────┘
...
[Figura 8.1: Ilustración tridimensional del Sistema de Entorno Espacial completo, mostrando todos los subsistemas principales y sus interconexiones.]

#### 8.2.2 Componentes Principales

[Figura 8.2: Despiece explosionado de la Unidad de Procesamiento Cuántico (QPU) y su sistema criogénico asociado.]

[Figura 8.3: Vista detallada del sistema de detección de radiación, mostrando los diferentes tipos de detectores y su disposición.]

[Figura 8.4: Ilustración del sistema LIDAR para detección de escombros, incluyendo componentes ópticos y electrónicos.]

#### 8.2.3 Instalación en la Aeronave

[Figura 8.5: Diagrama de instalación mostrando la ubicación de los componentes del sistema en una aeronave típica GAIA AIR.]
...
| TID | Total Ionizing Dose (Dosis Total de Ionización) |
| SEE | Single Event Effects (Efectos de Evento Único) |
| LET | Linear Energy Transfer (Transferencia Lineal de Energía) |
| GCR | Galactic Cosmic Rays (Rayos Cósmicos Galácticos) |
| SPE | Solar Particle Events (Eventos de Partículas Solares) |
